...
async def update(bot, seqno) -> None:
    ...
    try:
        ...
        prev_seqno = int(res.entry_value)
        if seqno > prev_seqno:
            ...
            logging.debug(f"verified a seqno out of order. not updating from {prev_seqno} to {seqno}")
    except Exception as e:
        # this functionality is entirely a nice-to-have, so I don't
        # really care if it errors
        logging.error(f"kvstore update error: {e}")
    return
...
async def fetch(bot) -> int:
    ...
    try:
        ...
        # this functionality is entirely a nice-to-have, so I don't
        # really care if it errors
        logging.error(f"kvstore fetch error: {e}")
        seqno = 0
    return seqno
